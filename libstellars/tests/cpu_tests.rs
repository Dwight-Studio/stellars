use serde_json::from_reader;
use std::fs::File;
use libstellars::Stellar;

macro_rules! generate_opcode_tests {
    ($($opcode:expr),+ ; $cycles_enabled:expr) => {
        $(
            paste::paste! {
                #[test]
                #[cfg(feature = "test-utils")]
                fn [<opcode_ $opcode>]() {
                    opcode_test($opcode, $cycles_enabled);
                }
            }
        )*
    };
}

#[cfg(feature = "test-utils")]
fn opcode_test(opcode: u8, cycles_enabled: bool) {
    let filename = format!("resources/cpu/{opcode:02x}.json");
    let file = File::open(&filename).unwrap_or_else(|_| panic!("Expected {filename}"));
    let json: serde_json::Value = from_reader(file).expect("File should be a json");

    for test in json.as_array().unwrap() {
        let initial_state = test.get("initial").unwrap().as_object().unwrap();
        let final_state = test.get("final").unwrap().as_object().unwrap();

        let stellar = Stellar::new();
        stellar.read().unwrap().set_initial_state(initial_state);

        stellar.read().unwrap().run_opcode();

        let (equal, differences) = if cycles_enabled {
            let cycles_values = test.get("cycles").unwrap().as_array().unwrap();
            stellar.read().unwrap().check_final_state(final_state, Some(cycles_values))
        } else {
            stellar.read().unwrap().check_final_state(final_state, None)
        };

        if !equal {
            eprintln!("Test \"{}\" with differences:", test.get("name").unwrap().as_str().unwrap());
            for diff in differences {
                eprintln!("  - {diff}");
            }
        }

        assert!(equal);
    }
}

/*generate_opcode_tests!(; true);*/

generate_opcode_tests!(0x93, 0x9b, 0x9c, 0x9e, 0x9f; true);